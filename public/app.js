...
const msg = (el, type, text) => { el.innerHTML = `<div class="${type}">${text}</div>`; };
...
if (classSelect && priceHint) {
  ...
  const upd = () => { priceHint.textContent = map[classSelect.value] || ''; };
...
if (upcomingBox) {
  (async () => {
    try {
      ...
      const j = await res.json();
      if (!j.ok) return;
      if (!j.matches.length) { upcomingBox.innerHTML = '<div class="notice">Zatím žádné zápasy pro příští měsíc.</div>'; return; }
    ...
    } catch (e) { upcomingBox.innerHTML = '<div class="notice">Nelze načíst kalendář.</div>'; }
  })();
}
...
if (loginBtn) {
  ...
  async function loadReservations() {
    ...
    const j = await res.json();
    if (!j.ok) { msg(loginMsg, 'error', j.error || 'Nelze načíst rezervace.'); return; }
    rezTbody.innerHTML = '';
  ...
  async function loadMatches() {
    ...
    const j = await res.json();
    if (!j.ok) { msg(mMsg, 'error', j.error || 'Nelze načíst zápasy.'); return; }
  ...
  loginBtn.addEventListener('click', async () => {
    ...
    const j = await res.json();
    if (j.ok) { loginBox.style.display = 'none'; adminArea.style.display = 'block'; await loadReservations(); await loadMatches(); }
    else { msg(loginMsg, 'error', j.error || 'Špatné heslo.'); }
  });
  ...
  matchForm.addEventListener('submit', async (e) => {
    ...
    const res = await fetch('/api/matches', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title, date }) });
    const j = await res.json();
